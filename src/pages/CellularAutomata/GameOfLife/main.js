...
export default {
  ...
  const offsetX = Math.floor(this.cols / 2) - Math.floor(gliderMachine.length / 2);
  const offsetY = Math.floor(this.rows / 2) - Math.floor(gliderMachine[0].length / 2);
    ...
    for (let j = 0; j < gliderMachine[0].length; j++) {
      ...
      for (let i = 0; i < this.cols; i++) {
        for (let j = 0; j < this.rows; j++) {
          ...
          ctx.fillRect(i * this.resolution, j * this.resolution, this.resolution - 1, this.resolution - 1);
        }
      }
    },
    update() {
